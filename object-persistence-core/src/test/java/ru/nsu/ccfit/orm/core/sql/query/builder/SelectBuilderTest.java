...
class SelectBuilderTest {
    ...
    private static Stream<Arguments> argsForSuperQuery() {
        return Stream.of(
            Arguments.of(
                List.of("{u}name", "{u}surname", "{u}age"),
                "users u",
                ConditionFactory.and(
                    ConditionFactory.or(
                        ConditionFactory.like("{u}name", "TEST1"),
                        ConditionFactory.notEquals("{u}age", 20)
                    ),
                    ConditionFactory.or(
                        ConditionFactory.like("{u}name", "TEST2"),
                        ConditionFactory.notEquals("{u}age", 19)
                    ),
                    ConditionFactory.equals("{u}surname", "TEST_SURNAME")
                ),
                ...
                "SELECT u.name AS u_name, u.surname AS u_surname, u.age AS u_age " +
                    "FROM users u " +
                    "LEFT JOIN users_films uf ON u.id = uf.id " +
                    "RIGHT JOIN users_books ub ON u.id = ub.id " +
                    "INNER JOIN users_music um ON u.id = um.id " +
                    "WHERE ((u.name LIKE ? OR u.age <> ?) AND (u.name LIKE ? OR u.age <> ?) AND u.surname = ?) " +
                    "ORDER BY u.name ASC " +
                    "LIMIT ?",
                List.of("TEST1", 20, "TEST2", 19, "TEST_SURNAME", 100)
            )
        );
    }
